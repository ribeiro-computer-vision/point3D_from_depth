...
def overlay_axes_p3d_no_figure(
    rgb: np.ndarray,
    cameras,
    height: int,
    width: int,
...
def plot_re_projected_uv_on_image(uv, rgb, H, W, cams):

    # ---- Project to screen --------------------------------------------------------
    device = cams.R.device
    dtype  = cams.R.dtype
    imgsz  = torch.tensor([[H, W]], device=device)
    ...
    overlay_axes_p3d_no_figure(rgb, cams, H, W,
                    world_origin=(0,0,0), axis_len=0.5,
                    draw_world_axes=True, draw_camera_axes=False,
                    cam_axis_len=0.5,
    ...
    for i, (u,v) in enumerate(uv):
        ...
        plt.text(u+5, v, f"{i}", fontsize=10, weight="bold",
                bbox=dict(facecolor="white", alpha=0.85, edgecolor="black", linewidth=0.5, pad=1.5))

    # Crosshair at principal point (for reference)
    cx = float(cams.principal_point[0,0]) if hasattr(cams, "principal_point") else W/2
    cy = float(cams.principal_point[0,1]) if hasattr(cams, "principal_point") else H/2
